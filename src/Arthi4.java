...
public class Arthi4 extends MainClass
{
	...
	Arthi4(boolean from_constructor,int line_Number)
	{
		is_in_scope=true;
		...
		Instruction_Sequence.set(line_Number, Instruction_Sequence.get(line_Number)+"A4        ");
		if(ID_.get(line_Number).return_is_instruction_Add())
			Reg_imm=get_ID_value2(line_number)+get_ID_value3(line_number);
		else if(ID_.get(line_Number).return_is_instruction_Subtraction())
			Reg_imm=get_ID_value2(line_number)-get_ID_value3(line_number);
